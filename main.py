...
if p.full_loader:
  ...
  p.SOS = module.source.vocab.stoi['<sos>']  # start of sentence token <sos> qes <eos> <pad...> <sos> <eos>
  ...
  vocab = torch.load(os.path.join(DATASET_TARGET_DIR,
                                  "vocab.pt"), map_location='cpu')
...
if p.load_model != "":
  if n_gpus == 1:
    ...
    state = torch.load(p.load_model, map_location='cpu')
...
# added for demo
import numpy as np
terminal = Terminal(model, eval_module, p)
it = iter(eval_module.iterator)

def print_random_sample():
  samples = next(it)
  random_idx = np.random.choice(samples.src.cpu().numpy().shape[0])
  x = samples.src.cpu().numpy()[[random_idx]]
  y = samples.trg.cpu().numpy()[[random_idx]]
  print("input: {}\ntarget:{}".format(eval_module.decode(x), eval_module.decode(y)))

print_random_sample()
print('AI is now ready for Q/A...')
...
def get_prediction():
  ...
  print(req['question'])
  ret = terminal.enter(str(req['question']))
  ret = re.search('<sos>(.*)<eos>',str(ret))
  return flask.jsonify({"ans":ret.group(1)})
